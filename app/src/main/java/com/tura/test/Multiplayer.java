...
public class Multiplayer extends AppCompatActivity {
    ...
    ImageView Carta1Down,Carta2Down,Carta3Down,Carta1Top,Carta2Top,Carta3Top,tx,top;



    TextView testoSopra, testoGiu, ResultText , puntiPlayerTop , puntiPlayerDown,coinUptext, coinDownText;
    int is0 = 0, is1 = 1 , is2 = 2, it1 = 3, it2 = 4 , it3 = 5, z = 39 ,nostraCarta=0;

    Animation[] animations = new Animation[16];

    Random rand = new Random();

    Boolean primo = true ,secondo = true,lose = true , turno = false, TurnoFinito = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("carte",idPartita);
        //per ottenere il nome della risorsa
        ilturno(idPlayer,idPartita,hand,c);
...
////
        if (turno) {
            Log.d("3h","inside if turno");
            enableCarts();
            updateTurno(idPlayer,idPartita,c);

        } else {
            Log.d("3h","inside else turno");
            disableCarts();
            updateTurno(idPlayer,idPartita,c);
        }
...
//                    Carta3Down.setEnabled(true);
    ...
    private void ottieniCarte(String idPartita,Context c){
        requestQueue = Volley.newRequestQueue(c);
        String url1 = url + "carte";
        Uri.Builder builder = Uri.parse(url1).buildUpon();
        builder.appendQueryParameter("idp", idPartita);
        ...
        StringRequest stringRequest = new StringRequest(
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        if (response!=null){
                            Log.d("carte",response);
                            String cleanedString = response.replaceAll("\\[|\\]", "");
                            String[] numberArray = cleanedString.split(",");
                            int index = 0;
                            // Converte in una lista di interi
                            for (String num : numberArray) {
                                if (index < numbers.length) { // Verifica di non superare i limiti dell'array
                                    numbers[index] = Integer.parseInt(num.trim());
                                    index++;
                                } else {
                                    break; // Interrompi il ciclo se l'array è pieno
                                }
                            }
                        }
                    }
    ...
    private void ilturno(String idplay, String idPartita,Hand hand,Context c) {
        requestQueue = Volley.newRequestQueue(c);
        String url1 = url + "turno";
        Uri.Builder builder = Uri.parse(url1).buildUpon();
        builder.appendQueryParameter("idp", idPartita);
        ...
        StringRequest stringRequest = new StringRequest(
                Request.Method.GET, urlWithParams,
                response -> {
                        if(response == null) handler.postDelayed(() -> ilturno(idplay,idPartita,hand,c), 1000);
                        else{
                            if(response.contentEquals(idplay)){
                                turno = true;
                                coinUptext.setText("x");
                                setCarta(idPartita,idplay,String.valueOf(is0),1,c);
                                ilturno(idplay,idPartita,hand,c);
//                            getCarta(idPartita);
//                            if(!cartaAvversario.contentEquals("x")){
//                                DrawCard(Carta1Top,Integer.parseInt(response),hand);
//                            }
//                            playTurn(idplay,idPartita);
                            }
                            else {
                                Log.d("3H","else");
                                turno = false;
                                coinUptext.setText("H");
//                            new Handler().postDelayed(new Runnable() {
//                                @Override
//                                public void run() {
//                                    ng.setVisibility(View.VISIBLE);
//                                    ng.setEnabled(false);
//                                    ng.setText("turno del avversario");
//                                    Log.d(" ","non e' tuo turno");
//                                    ilturno(idplay,idPartita,hand);
//                                }
//                            }, 500);
                            }
                        }
    ...
    private void setCarta(String idPartita,String idPlayer,String carta,int pos,Context c){
        disableCarts();
        TurnoFinito = false;
        requestQueue = Volley.newRequestQueue(c);
        String url1 = url + "setCarta";
        Uri.Builder builder = Uri.parse(url1).buildUpon();
        builder.appendQueryParameter("idp", idPartita);
        builder.appendQueryParameter("carta", carta);
        builder.appendQueryParameter("player", idPlayer);
        //builder.appendQueryParameter("param2", "value2");
        String urlWithParams = builder.build().toString();
        ...
        StringRequest stringRequest = new StringRequest(
                ...
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        if(response.contentEquals("100")) getCarta(idPartita,c);
                    }
                },
                new Response.ErrorListener() {
                    @Override
...
//            }});
        menuB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent i = new Intent(Multiplayer.this, MenuPrincipale.class);
